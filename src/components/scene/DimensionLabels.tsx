...
export function DimensionLabels() {
  const { room, showDimensions } = useConfigurator((s) => ({
    room: s.room,
  ...
  return (
    <group>
      {/* Width dimension along front of opening */}
      <Line
        points={[
      ...
      />
      <Html
        position={[0, yLine, d / 2 + offset + 0.6]}
        center
        zIndexRange={[100, 0]}
        distanceFactor={14}
      >
      ...
      {/* Depth dimension along right wall */}
      <Line
        points={[
          [w / 2 + offset, yLine, -d / 2],
          [w / 2 + offset, yLine, d / 2],
        ]}
        color="#9bd1ff"
        lineWidth={1.5}
      />
      <Html
        position={[w / 2 + offset + 0.6, yLine, 0]}
        center
        zIndexRange={[100, 0]}
        distanceFactor={14}
      >
      ...
      {/* Height dimension along left rear */}
      <Line
        points={[
          [-w / 2 - offset, 0, -d / 2],
          [-w / 2 - offset, h, -d / 2],
      ...
      />
      <Html
        position={[-w / 2 - offset - 0.4, h / 2, -d / 2]}
        center
        zIndexRange={[100, 0]}
        distanceFactor={14}
      >
